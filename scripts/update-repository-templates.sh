...
function update {
    ...
    sed -i '' -e "s|{{Project}}|$humanName|g" `find "$workdir/.github" -type f -print` "$workdir/CONTRIBUTING.md" "$workdir/SECURITY.md" "$workdir/.github/ISSUE_TEMPLATE/config.yml"
    sed -i '' -e "s|{{Repo}}|$name|g" `find "$workdir/.github" -type f -print` "$workdir/.github/ISSUE_TEMPLATE/config.yml"
