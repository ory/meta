...
// fizzMigrationsCreateCmd represents the create command
var fizzMigrationsCreateCmd = &cobra.Command{
	Use:  "create [name]",
...
func init() {
	fizzMigrationsCmd.AddCommand(fizzMigrationsCreateCmd)

	fizzMigrationsCreateCmd.Flags().StringP("dialect", "d", "", fmt.Sprintf(`Choose a dialect from: %v`, supportedDialects))
}
